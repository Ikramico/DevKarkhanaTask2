...
//problem 1
const Above50 = data.filter(d => (d.point >= 50));
let Output = [];
Above50.map(person => {
    Output.push({
...
console.log('Users Having minimum 50 points:');
console.table(Output.sort(({ Point: a }, { Point: b }) => b - a));

//problem 2
console.log(`Users with point more than 20 and less then 50 = ${data.filter(d => (d.point > 20) && (d.point < 50)).length}`);

//problem 3
const User = data.find(d => d.email === 'Tristian7@yahoo.com');
console.log(`Point of ${User.email} is ${User.point}`);
